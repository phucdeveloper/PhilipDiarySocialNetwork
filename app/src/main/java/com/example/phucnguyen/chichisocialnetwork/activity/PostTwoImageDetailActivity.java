...
public class PostTwoImageDetailActivity extends AppCompatActivity {

    TextView txtContentPost, txtNameAccount, txtTimeCreate;
    ImageView imgAvatar, imgDisplayImage, imgAvatarComment, imgOne, imgTwo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_post_two_image_detail);
        initView();
        ...
        if (timeline == null) {
            Toast.makeText(PostTwoImageDetailActivity.this, "Khong nhan duoc du lieu nao ca", Toast.LENGTH_SHORT).show();
        } else {
            idPost = timeline.getPostImage().getIdPost();
            Glide.with(PostTwoImageDetailActivity.this).load(userPost.getAvatar()).into(imgAvatar);
            txtTimeCreate.setText(timeline.getPostImage().getTimeCreate());
            txtNameAccount.setText(userPost.getNickname());
            Glide.with(PostTwoImageDetailActivity.this).load(user.getAvatar()).into(imgAvatarComment);
            Glide.with(PostTwoImageDetailActivity.this).load(timeline.getPostImage().getArrayList().get(0)).into(imgOne);
            Glide.with(PostTwoImageDetailActivity.this).load(timeline.getPostImage().getArrayList().get(1)).into(imgTwo);
        }
    ...
    private void setUpRecyclerViewListComment() {
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(PostTwoImageDetailActivity.this, RecyclerView.VERTICAL, true);
        ...
        if (idPost != null) {
            ...
            databaseReference.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    CommentAdapter adapter = new CommentAdapter(arrayList, PostTwoImageDetailActivity.this);
                    recyclerViewListComment.setAdapter(adapter);
                }

                @Override
                public void onCancelled(@NonNull DatabaseError databaseError) {
                    Toast.makeText(PostTwoImageDetailActivity.this, databaseError.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
